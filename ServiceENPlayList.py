...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.serviceEN.vn")
pluginrootpath = "plugin://plugin.video.serviceEN.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.serviceEN.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.serviceEN.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.serviceEN.vn" in item["path"]:
