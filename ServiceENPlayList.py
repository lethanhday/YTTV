...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.service.vn")
pluginrootpath = "plugin://plugin.video.service.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.service.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.service.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.service.vn" in item["path"]:
