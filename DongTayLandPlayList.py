...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.shopping.vn")
pluginrootpath = "plugin://plugin.video.shopping.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.shopping.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.shopping.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.shopping.vn" in item["path"]:
