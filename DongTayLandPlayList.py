...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.DongTayLand.vn")
pluginrootpath = "plugin://plugin.video.DongTayLand.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.DongTayLand.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.DongTayLand.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.DongTayLand.vn" in item["path"]:
