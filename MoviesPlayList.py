...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.Movies.vn")
pluginrootpath = "plugin://plugin.video.Movies.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.Movies.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.Movies.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.Movies.vn" in item["path"]:
