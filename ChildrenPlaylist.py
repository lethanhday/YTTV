...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.children.vn")
pluginrootpath = "plugin://plugin.video.children.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.children.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.children.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.children.vn" in item["path"]:
