...
plugin = Plugin()
addon          = xbmcaddon.Addon("plugin.video.edu.vn")
pluginrootpath = "plugin://plugin.video.edu.vn"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.edu.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.edu.vn(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.edu.vn" in item["path"]:
